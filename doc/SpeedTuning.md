...
> For the statistics test, we subsampled the trials to reduce speed-distribution imbalance by `_subsample()`
...
Across the three stimuli, the fewest neurons is tuned to running under `drifting_gratings`, while the most under `static_gratings`. It suggests that the speed tunings of the 47 V1 neurons are stimuli-dependent.

Interestingly, there are two neurons showing tuned by running speed even during `spontaneous`.
...
Here we show the tuning curves of neurons grouped by monotonicity across 3 stimuli, along with the basline of the same neurons' tuning under `spontaneous`.

The lines linking dots denotes the average responses and the shadow intervals denotes the MSE, where solid dots with real lines denote the specific stimuli, while blank dots with dash lines denote the `spontaneous` tuning. Average Spearman's $\rho$ of the selected neurons is also shown.
...
![](figures/tuning_by_monotonicity_with_spont.png)


### 2.4 Each neuron's tuning
Here we dive into each neuron's tuning monotonicity across different stimuli conditions, along with the `responsive` and `speed-tuned` mask based on the metadata from Allen's : [`neurons_metadata.csv`](../data/neurons_metadata.csv). 


An outstanding neuron (\# 25) that is always positively running-tuned.
...
*(to add the tuned mask from `BinaryModulation`)*
