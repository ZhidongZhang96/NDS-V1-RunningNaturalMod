...
def _download_session(boc, exp_id, letter):
    ...
    try:
        ...
        # Fallback: one epoch per stimulus, gaps between stimulus blocks = spontaneous
        log("    (epoch table unavailable, building fallback)")
        rows = []
        for stim_name, df in stim_tables.items():
            rows.append({"stimulus": stim_name,
        ...
        for row in rows:
            if row["start"] > prev_end:
                epoch_rows.append({"stimulus": "spontaneous",
                                   "start": prev_end, "end": row["start"]})
